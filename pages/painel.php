...
// Redireciona o administrador para a tela de gerenciamento de produtos
// SOMENTE quando ele entra no painel sem escolher uma área específica.
//
// Antes esta condição também bloqueava "?area=admin", o que criava um
// problema: o administrador conseguia abrir as áreas de suporte, moderação
// etc., mas NÃO conseguia abrir a própria área de administração (a página
// redirecionava em vez de mostrar o conteúdo).
// Agora só redireciona quando NENHUMA área foi informada.
if ($userRoleKey === 'admin' && !isset($_GET['area'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrf_check($_POST['csrf'] ?? null)) {
        ...
        if (!validar_chave_mestre_usuario($userId, $senhaMaster)) {
            $mensagem = 'ðŸ”’ Chave Mestre de confirmação incorreta para a sua conta (' . e($user['nome']) . ')! Operação cancelada por segurança.';
            $tipoMensagem = 'error';
        } else {
            switch ($acao) {
...
<div class="section">
    <div class="section-head" style="margin-bottom: 20px;">
        ...
        <div style="text-align: right;">
            <span style="display: inline-block; padding: 6px 14px; background: rgba(34, 197, 94, 0.2); border: 1px solid rgba(197, 34, 34, 0.44); border-radius: 20px; font-weight: 700; color: var(--text);">
    ...
    <div style="background: var(--testo2); border: 1px solid var(--border); border-radius: 12px; padding: 14px 18px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 10px;">
        <div>
            <strong style="font-size: 0.92rem; color: var(--text2);">Sua Chave Mestre Pessoal de Confirmação</strong>
        ...
        <form method="post" action="<?= e($panelAction) ?>" style="display: flex; gap: 8px; align-items: center;">
            ...
            <input type="password" name="senha_master" required minlength="4" placeholder="Chave atual" style="padding: 6px 10px; font-size: 0.82rem; border-radius: 6px;  border: 1px solid var(--border); color: var(--text);">
            <input type="password" name="nova_chave_mestre" required minlength="4" placeholder="Nova chave mestre" style="padding: 6px 10px; font-size: 0.82rem; border-radius: 6px;  border: 1px solid var(--border); color: var(--text);">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            $rBadge = $rolesInfo[$uTipo]['badge'] ?? 'ðŸ›’ Cliente';
                                        ?>
    ...
    <?php elseif ($viewRoleKey === 'developer'): ?>
        <div class="panel" style="border-left: 4px solid #06b5d4dc;">
            <h2 style="color: var(--info-text);">ðŸ› ️ Painel do Desenvolvedor &amp; Engenharia de Sistemas</h2>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 14px; margin-top: 16px;">
                <div style=" padding: 14px; border-radius: 8px; border: 1px solid rgba(6,182,212,0.3);">
                    <small style="color: var(--info-text);">Versão PHP</small>
                    <div style="font-size: 1.2rem; font-weight: 700; color: var(--text);"><?= PHP_VERSION ?></div>
                </div>
                <div style=" padding: 14px; border-radius: 8px; border: 1px solid  rgba(6, 181, 212, 0.59);">
                    <small style="color: var(--info-text);">Servidor Web</small>
                    <div style="font-size: 0.92rem; font-weight: 700; color: var(--text);"><?= e($_SERVER['SERVER_SOFTWARE'] ?? 'Apache') ?></div>
                </div>
                <div style=" padding: 14px; border-radius: 8px; border: 1px solid rgba(6, 181, 212, 0.59);">
                    <small style="color: var(--info-text);">Status Banco MySQL</small>
                    <div style="font-size: 1.1rem; font-weight: 700; color: var(--text);">ðŸŸ¢ Ativo e Conectado</div>
                </div>
                <div style=" padding: 14px; border-radius: 8px; border: 1px solid  rgba(6, 181, 212, 0.59);">
                    <small style="color: var(--info-text);">Sessão PHP</small>
                    <div style="font-size: 0.85rem; font-weight: 600; color: var(--text); overflow: hidden; text-overflow: ellipsis;"><?= e(session_id()) ?></div>
                </div>
            </div>

            <h3 style="margin-top: 24px; color: var(--info-text);">Contagem de Registros por Tabela MySQL</h3>
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); gap: 10px; margin-top: 12px;">
                <?php
                    ...
                    foreach ($tables as $tbl) {
                        ...
                        echo "<div style='; padding: 10px 14px; border-radius: 8px; border: 1px solid var(--border); display:flex; justify-content:space-between; align-items:center;'>";
                        echo "<span style='font-size:0.85rem; color:var(--muted);'>$tbl</span>";
                        echo "<strong style='color:var(--success); font-size:1rem;'>$cnt</strong>";
                        echo "</div>";
                    }
                ?>
            </div>
            ...
            <div style="margin-top: 24px;">
                <form method="post" action="<?= e($panelAction) ?>">
                    ...
                    <div style="display:flex; gap:10px; align-items:center; flex-wrap:wrap;">
                        <input type="password" name="senha_master" required placeholder="ðŸ”’ Sua Chave Mestre Desenvolvedor" style="padding: 8px 12px; border-radius: 6px; border: 1px solid var(--border); color:var(--text);">
                        <button type="submit" class="btn btn-sm">âš¡ Limpar Cache e Re-sincronizar Carrinho</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <?php elseif ($viewRoleKey === 'support'): ?>
        <div class="panel" style="border-left: 4px solid #3b82f6;">
            <h2 style="color: var(--blue-text);">ðŸŽ§ Central de Atendimento &amp; Suporte ao Cliente</h2>
            <p class="sub">Atenda chamados, consulte compras de clientes e atualize informações de atendimento.</p>

            <h3 style="margin-top: 20px; color: var(--blue-text);">Chamados de Atendimento Recebidos (<?= count($chamadosSuporte) ?>)</h3>
                    ...
                    <tbody>
                        <?php foreach ($chamadosSuporte as $cham): ?>
                            <tr>
                                ...
                                <td style="max-width:280px; font-size:0.85rem;"><?= e($cham['mensagem']) ?><?php if (!empty($cham['resposta'])): ?><br><small style="color:var(--success);"><strong>Resposta enviada:</strong> <?= e($cham['resposta']) ?></small><?php endif; ?></td>
                                ...
                                <td>
                                        ...
                                        <input type="text" name="resposta" placeholder="Digite a resposta do suporte..." required style="padding:4px 8px; font-size:0.8rem; border-radius:4px;  border:1px solid var(--border); color:var(--text);">
                                        <input type="password" name="senha_master" placeholder="ðŸ”‘ Sua Chave Mestre" required style="padding:4px 8px; font-size:0.8rem; border-radius:4px; border:1px solid var(--border); color: var(--text);">
    ...
    <?php elseif ($viewRoleKey === 'moderator'): ?>
        <div class="panel" style="border-left: 4px solid #a855f7;">
            <h2 style="color: var(--purple-text);">ðŸ›¡️ Central de Moderação de Conteúdo &amp; Comunidade</h2>
            <p class="sub">Aprove ou rejeite avaliações de produtos e modere publicações de clientes.</p>

            <h3 style="margin-top: 20px; color: var(--purple-text);">Avaliações de Produtos para Moderação (<?= count($avaliacoesProdutos) ?>)</h3>
            <div class="admin-table-responsive" style="margin-top: 10px;">
                <table class="admin-table">
                    <thead>
                    ...
                    <tbody>
                        <?php foreach ($avaliacoesProdutos as $aval): ?>
                            <tr>
                                ...
                                <td style="max-width:250px; font-size:0.85rem;"><?= e($aval['comentario']) ?><?php if (!empty($aval['denuncias_info'])): ?><br><small style="color:var(--danger-text);">Denúncia: <?= e($aval['denuncias_info']) ?></small><?php endif; ?></td>
                                ...
                                <td>
                                    <form method="post" action="<?= e($panelAction) ?>" style="display:flex; gap:6px; align-items:center;">
                                        ...
                                        <input type="password" name="senha_master" placeholder="ðŸ”‘ Sua Chave Mestre" required style="padding:4px 8px; font-size:0.8rem; border-radius:4px; border:1px solid var(--border); color: var(--text); width:130px;">
    ...
    <?php elseif ($viewRoleKey === 'manager'): ?>
        <div class="panel" style="border-left: 4px solid #10b981;">
            <h2 style="color: var(--teal-text);">ðŸ“¦ Gestão Comercial, Estoque e Promoções</h2>
            ...
            <div style="background: rgba(16,185,129,0.1); border: 1px solid rgba(16,185,129,0.3); border-radius: 12px; padding: 16px; margin-top: 16px;">
                <h3 style="color: var(--teal-text); font-size: 1rem; margin-bottom: 10px;">ðŸŽŸ️ Cadastrar Novo Cupom de Desconto</h3>
                <form method="post" action="<?= e($panelAction) ?>" class="manager-coupon-form">
                    <input type="hidden" name="csrf" value="<?= e(csrf_token()) ?>">
                    <input type="hidden" name="acao" value="adicionar_cupom">
                    <input type="text" name="codigo_cupom" placeholder="Ex: PROMO15" required style="padding:8px 12px; border-radius:6px;  border:1px solid var(--border); color:var(--text); text-transform:uppercase;">
                    <input type="number" name="desconto_percentual" placeholder="Desconto %" min="1" max="90" required style="padding:8px 12px; border-radius:6px;  border:1px solid var(--border); color:var(--text); width:120px;">
                    <input type="password" name="senha_master" placeholder="ðŸ”‘ Sua Chave Mestre" required style="padding:8px 12px; border-radius:6px; border:1px solid var(--border); color: var(--text);">
                    <button type="submit" class="btn btn-sm">Criar Cupom</button>
                </form>
            </div>

            <h3 style="margin-top: 24px; color: var(--teal-text);">Cupons Ativos na Loja</h3>
            <div style="display:flex; gap:10px; flex-wrap:wrap; margin-top:8px;">
                <?php foreach ($cuponsLoja as $cup): ?>
                    <div class="manager-coupon-card">
                        <strong style="color:var(--success);"><?= e($cup['codigo']) ?></strong> â” <?= (float)$cup['desconto_percentual'] ?>% OFF
            ...
            <div class="panel-head-flex" style="margin-top: 24px;">
                ...
                <button type="button" class="btn btn-sm" onclick="document.getElementById('manager-new-product').scrollIntoView({ behavior: 'smooth' })">âž• Cadastrar Produto</button>
            </div>
            <div class="admin-table-responsive">
                <table class="admin-table">
                    <thead>
    ...
    <?php elseif ($viewRoleKey === 'financial'): ?>
        <div class="panel" style="border-left: 4px solid #f59e0b;">
            <h2 style="color: var(--warn-text);">ðŸ’° Painel Financeiro &amp; Balanço Comercial</h2>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; margin-top: 16px;">
                <div style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; padding: 18px;">
                    <small style="color: var(--warn-text); font-weight: 600;">Faturamento Bruto Total em Vendas</small>
                    <div style="font-size: 1.6rem; font-weight: 700; color: var(--text); margin-top: 4px;"><?= e(money($faturamentoTotal)) ?></div>
                </div>
                <div style="background: rgba(245,158,11,0.1); border: 1px solid rgba(245,158,11,0.3); border-radius: 12px; padding: 18px;">
                    <small style="color: var(--warn-text); font-weight: 600;">Vendas Confirmadas</small>
                    <div style="font-size: 1.6rem; font-weight: 700; color: var(--success); margin-top: 4px;"><?= $qtdPedidosPagos ?> pedidos</div>
                </div>
            </div>

            <h3 style="margin-top: 24px; color: var(--warn-text);">Transações de Pedidos e Ações Financeiras</h3>
            <div class="admin-table-responsive" style="margin-top: 10px;">
                <table class="admin-table">
                    <thead>
                        ...
                        <?php foreach ($pedidos as $ped): ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php else: ?>
                                        <small style="color:var(--success);">Reembolso concluído</small>
                                    <?php endif; ?>
                                </td>
                            </tr>
                        <?php endforeach; ?>
    ...
    <?php elseif ($viewRoleKey === 'logistics'): ?>
        <div class="panel" style="border-left: 4px solid #f97316;">
            <h2 style="color: var(--orange-text);">ðŸšš Central de Logística &amp; Expedição de Cargas</h2>
            ...
            <h3 style="margin-top: 20px; color: var(--orange-text);">Fila de Expedição de Pedidos</h3>
            <div class="admin-table-responsive" style="margin-top: 10px;">
                <table class="admin-table">
                    <thead>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php if (!empty($ped['rua'])): ?>
                                        <small><?= e($ped['rua']) ?>, nº <?= e($ped['numero']) ?> â” <?= e($ped['cidade']) ?>/<?= e($ped['estado']) ?> (CEP: <?= e($ped['cep']) ?>)</small>
                                    <?php endif; ?>
                                </td>
                                <td><span class="badge-status status-pago"><?= e($ped['status']) ?></span></td>
                                <td>
                                    <form method="post" action="<?= e($panelAction) ?>" style="display:flex; flex-direction:column; gap:6px;">
                                        ...
                                        <div style="display:flex; gap:6px;">
                                            ...
                                            <input type="text" name="codigo_rastreio" placeholder="Ex: TF123456789BR" value="<?= e($logisticaAtual['codigo_rastreio'] ?? '') ?>" required style="padding:4px 8px; font-size:0.8rem; border-radius:4px;  border:1px solid var(--border); color:var(--text); width:140px;">
                                            <select name="status_expedicao" style="padding:4px 8px; font-size:0.8rem; border-radius:4px; background: var(--field-bg); border:1px solid var(--border); color: var(--text);">
...
<div id="modalManagerEditProduct" class="checkout-message hidden" role="dialog" aria-modal="true">
    <div class="checkout-message-card admin-modal-card">
        <button type="button" class="checkout-close" onclick="closeModal('modalManagerEditProduct')">Ã—</button>
...
<div id="modalEditOrder" class="checkout-message hidden" role="dialog" aria-modal="true">
    <div class="checkout-message-card">
        <button type="button" class="checkout-close" onclick="closeModal('modalEditOrder')">Ã—</button>
        ...
        <form method="post" action="<?= e($panelAction) ?>">
            ...
            <div class="field" style="margin-bottom: 14px;">
                ...
                <select id="edit_order_status" name="status" required style="width: 100%; padding: 10px; border-radius: 8px; color: var(--text); border: 1px solid var(--border);">
            ...
            <div class="field-master-security" style="margin-bottom: 16px;">
                <label for="edit_order_master">ðŸ”’ Sua Chave Mestre de Confirmação</label>
...
<div id="modalAddUser" class="checkout-message hidden" role="dialog" aria-modal="true">
    <div class="checkout-message-card admin-modal-card">
        <button type="button" class="checkout-close" onclick="closeModal('modalAddUser')">Ã—</button>
        <h3>âž• Cadastrar Novo Usuário e Cargo</h3>
        <form method="post" action="<?= e($panelAction) ?>" class="form-grid">
            ...
            <div class="field" style="grid-column: 1 / -1;">
                ...
                <select id="add_user_tipo" name="tipo" required style="background: rgba(16, 3, 5, 0.45); border: 1px solid var(--border); color: var(--text); border-radius: 10px; padding: 12px 14px; width: 100%;">
                    <option value="customer">ðŸ›’ 8. Cliente (Acesso comum a compras e perfil)</option>
                    <option value="admin">ðŸ‘‘ 1. Administrador (Controle total da plataforma)</option>
                    <option value="developer">ðŸ› ️ 2. Desenvolvedor (Acesso técnico, logs e configurações)</option>
                    <option value="support">ðŸŽ§ 3. Suporte (Atendimento, consulta de pedidos e entregas)</option>
                    <option value="moderator">ðŸ›¡️ 4. Moderador (Moderação de conteúdo e suspensão de usuários)</option>
                    <option value="manager">ðŸ“¦ 5. Gerente de Loja (Produtos, estoque, categorias e promoções)</option>
                    <option value="financial">ðŸ’° 6. Financeiro (Acompanhamento financeiro e reembolsos)</option>
                    <option value="logistics">ðŸšš 7. Logística (Expedição, rastreamento e status de envio)</option>
                </select>
            </div>
            <div class="field-master-security" style="grid-column: 1 / -1;">
                <label for="add_user_master">ðŸ”’ Sua Chave Mestre de Confirmação</label>
